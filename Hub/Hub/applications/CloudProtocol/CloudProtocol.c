...
u8 *ReplyDeviceListDataToCloud(mqtt_client *client, u8 *cloudRes, u8 sendCloudFlg)
{
    char        name[20];
    char        *str        = RT_NULL;
...
//                *len = 0;
    ...
    return 0;//Justin debug
}

/**
 * 发布数据(回复云服务器)
 */
u8 *ReplyDataToCloud1(mqtt_client *client, u8 *cloudRes, u16 *len, u8 sendCloudFlg)//Justin debug 增加是否发送结束
{
    char        name[20];
    char        *str        = RT_NULL;
    u8          *page       = RT_NULL;
